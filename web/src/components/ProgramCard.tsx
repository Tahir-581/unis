...
import type { ProgramRow, RejectionReason, UserApplication, UserOverride } from "@/lib/types";
import { REJECTION_LABELS, RejectModal } from "@/components/RejectModal";
import { categoryLabel, domainLabel } from "@/lib/taxonomy";
import { domainBadgeClass } from "@/components/FiltersBar";
import {
  countryFlag,
  daysUntil,
  effectiveDeadline,
...
interface Props {
  ...
  view: "programs" | "rejected";
  onToggleDone: (done: boolean) => void;
  onNotesChange: (notes: string) => void;
  onOverride: (patch: UserOverride) => void;
  onReject: (reason: RejectionReason, document?: string, note?: string) => void;
  onRestore: () => void;
}
...
export function ProgramCard({
  ...
  const override = application.overrides;
  const done = status === "done";
  const rejected = status === "rejected";

  const deadline =
    override?.application_deadline_non_eu ?? effectiveDeadline(program);
    ...
    <>
      <article
        className={`rounded-xl border p-4 transition ${
          rejected
            ? "border-red-200 bg-red-50/30 opacity-90 dark:border-red-900/40 dark:bg-red-950/20"
            : done
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            {(program.field_domain || program.study_category) && (
              <div className="mt-1 flex flex-wrap items-center gap-2">
                {program.field_domain && (
                  <span
                    className={`rounded px-1.5 py-0.5 text-xs font-medium ${domainBadgeClass(program.field_domain)}`}
                  >
                    {domainLabel(program.field_domain)}
                  </span>
                )}
                {(program.study_category || program.cs_category) && (
                  <span className="text-xs text-zinc-500">
                    {categoryLabel(program.study_category || program.cs_category)}
            ...
            {view === "programs" && !rejected && (
              <>
                ...
                <button
                  ...
                  className="rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-50 dark:border-red-900 dark:text-red-400 dark:hover:bg-red-950/30"
                >
                  Reject
                </button>
              </>
        ...
        {rejected && application.rejection_reason && (
          <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm dark:border-red-900/40 dark:bg-red-950/30">
            <span className="font-medium text-red-800 dark:text-red-300">
              Rejected: {REJECTION_LABELS[application.rejection_reason]}
            </span>
            {application.rejection_document && (
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Field label="Confidence" value={<span className="capitalize">{program.confidence}</span>} />
          <Field
            label="Last verified"
            value={
      ...
      {showRejectModal && (
        <RejectModal
          ...
          onConfirm={(reason, document, note) => {
